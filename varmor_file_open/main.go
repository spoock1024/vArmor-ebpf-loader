...
func main() {
	...
	if output, err := openFileTrigger(); err != nil {
		if strings.Contains("Operation not permitted", output) {
			fmt.Println("Operation not permitted，file rules work well")
		}
	}

}

func openFileTrigger() (string, error) {
	// open /etc/hostname
